...
image readImage::read(const char* nameFile) {
	...
	picture.bitmap = new PIXEL*[picture.info.depth];
	for (int i = 0; i < picture.info.depth; i++)
		picture.bitmap[i] = new PIXEL[picture.info.width];
...
image resizeImage::resize(image& picture, double coefficient) {
	...
	resizePicture.bitmap = new PIXEL*[resizePicture.info.depth];
	for (int i = 0; i < resizePicture.info.depth; i++)
		resizePicture.bitmap[i] = new PIXEL[resizePicture.info.width];
